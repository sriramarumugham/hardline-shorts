...
// Per-video branding, supplied via input props (see specs/*.json).
export type Brand = { kicker: string; date: string; logo: string; name: string };
...
// Slow Ken Burns zoom on the backing image.
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{
            // fall back to system sans for the ↘ glyph (not in Noto Tamil)
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
        ...
        >
          <Img
            src={staticFile(`images/${brand.logo}`)}
...
const StatCard: React.FC<{ stat: NonNullable<Stat> }> = ({ stat }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
